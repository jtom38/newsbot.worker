...
from newsbot.core.sql import Articles, ArticlesTable, IconsTable, Icons, Sources
from newsbot.worker.sources.common import BaseSources, ISources
from newsbot.worker.sources.common.parserExceptions import ParseUnableToFindAuthorName, ParserUnableToFindDescription
...
class RssReader(BaseSources, ISources):
    def __init__(self) -> None:
        self.logger = Logger(__class__)
        self.settings = RssSettings()
        self.uri = "https://example.net/"
        self.setSiteName(SourceName.RSS)
        self.feedName: str = ""
    ...
    def getArticles(self) -> List[Articles]:
        ...
        for link in self.__links__:
            ...
            self.logger.debug(f"{link.source} - {link.name} - Checking for updates")
            self.setActiveSource(source=SourceName.RSS, name=link.name)
        ...
        if desc != "":
            ...
            for i in items:
                try:
                    ...
                    if i != "":
                        return i
                except ParserUnableToFindDescription as e:
                    self.logger.warning(f"Unable to find the description of the post with the known items. {e}")
                    pass
    ...
    def findItemAuthor(self, item: BeautifulSoup) -> str:
        ...
        for i in items:
            try:
                ...
                if itemAuthor != "":
                    return itemAuthor
            except ParseUnableToFindAuthorName as e:
                self.logger.warning(f"Was unable to find the author on the RSS feed against the known items. {e}")
                pass
