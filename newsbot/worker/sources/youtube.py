...
from newsbot.core.sql import SourceLinks, Articles, Sources, DiscordWebHooks
from newsbot.core.logger import Logger
from newsbot.core.constant import SourceName
from newsbot.worker.sources.common import BaseSources, BFirefox, ISources


class YoutubeReader(BaseSources, BFirefox, ISources):
    def __init__(self):
        self.logger = Logger(__class__)
        self.cache = Cache()
        self.uri: str = "https://youtube.com"
        self.setSiteName(SourceName.YOUTUBE)
    ...
    def getArticles(self) -> List[Articles]:
        ...
        for site in self.__links__:
            self.setActiveSource(source=SourceName.YOUTUBE, name=site.name)
            self.authorName = ""
            self.authorImage = ""
